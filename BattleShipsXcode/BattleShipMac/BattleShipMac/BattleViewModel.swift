...
class BattleViewModel {
    ...
    let yDim = 10
    fileprivate let allyPlayerId = Battle.PlayerId.Player1
    fileprivate let enemyPlayerId = Battle.PlayerId.Player2
    fileprivate var vBondAlly: BondPlayer // contains listeners, to inform controllers of changes
    fileprivate var vBondEnemy: BondPlayer
    fileprivate var vRandomIndexs: [Int] = []
    fileprivate var vEnemyBattleStart: Battle // Battle that just has the enemy ship. Ally ships not yet deployed
    fileprivate var vBattle: Battle
    ...
    func addUpdateListener(#imThePlayer: _ Bool, _ updateListener: ([Int]) -> Void) {
    ...
    fileprivate func shootAtPlayerId(_ playerId: Battle.PlayerId, index: Int) -> Battle.BattleOperation {
        let y = index / xDim
        let x = index % xDim
        let battleOp = vBattle.shootAtPlayerId(playerId, y: y, x: x)
        print("\(playerId) \(battleOp.message)")
        return battleOp
    }
    ...
    func shootIndex(_ index: Int) {
        ...
        print("RANDCount: \(vRandomIndexs.count)")
    ...
    fileprivate func nearbyWaterForHit(_ hitIndex: Int, randomIndexes: [Int], board1D: [Battle.SeaScape]) -> [Int] {
        ...
        var randomIndexesWithout = randomIndexes.filter {!contains(indexes)(indexes,$0)}
    ...
    fileprivate class BondPlayer {
        let playerId: Battle.PlayerId
        fileprivate(set) var vBoard1D: [Battle.SeaScape]
        ...
        init (battle: Battle, playerId: Battle.PlayerId) {
            ...
            vBoard1D = board.reduce([], +)
        }
        
        func updateBondArray(_ battle: Battle) {
            let board = battle.boardForPlayerId(playerId)
            var arr = board.reduce([], +)
            var vIndexes: [Int] = []
